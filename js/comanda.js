...
async function cargarMenuCompleto() {
    try {
        const res = await fetch('https://localhost:7266/api/v1/Dish');
        if (res.ok) {
...
async function cargarComandas() {
    ...
    try {
        ...
        const res = await fetch('https://localhost:7266/api/v1/Order');
        if (!res.ok) throw new Error('Error al cargar comandas');
        
        comandas = await res.json();
...
function mostrarComandas(lista) {
    ...
    contenedor.innerHTML = lista.map(c => `
        <div class="comanda-card" onclick="verDetallesComanda(${c.orderNumber})">
...
async function verDetallesComanda(orderId) {
    try {
        const res = await fetch(`https://localhost:7266/api/v1/Order/${orderId}`);
        if (!res.ok) throw new Error('Error al cargar la comanda');
        
        comandaActual = await res.json();
...
function agregarMasPlatos() {
    ...
    sessionStorage.setItem('comandaActiva', comandaActual.orderNumber.toString());
    window.location.href = 'menu.html';
}

function mostrarCargando(mostrar) {
    document.getElementById('cargando-comandas').style.display = mostrar ? 'block' : 'none';
}

function mostrarError(mensaje) {
    document.getElementById('lista-comandas').innerHTML = `<p class="error">${mensaje}</p>`;
}
